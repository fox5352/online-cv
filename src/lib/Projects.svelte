<script>
	...
	async function getRepos() {
		...
		languageMenu = [
			...new Set(filteredData.map((data) => data.language).filter((data) => data != null))
		];
	...
	async function filterRepos(date, language) {
		...
		if (language !== 'all') {
			newList = data.filter(
				(listItem) => listItem.language?.toLowerCase() == language.toLowerCase()
			);
		}
			...
			<div transition:fade class="repo-container" id="projects">
				{#each repos as repo}
					{#if repo.language}
						<a
							class="repo-cell"
							href={`https://github.com/fox5352/${repo.name}`}
							aria-label={`repository to ${repo.name} project`}
							target="_blank"
						>
							<!-- load name -->
...
<style>
	...
	.repo-cell {
		...
		box-shadow:
			0 4px 6px -1px var(--ac-one),
			0 2px 4px -2px var(--ac-one);
	...
	@media screen and (min-width: 640px) {
		...
		.repo-cell {
			box-shadow:
				0 1px 3px 0 var(--ac-one),
				0 1px 2px -1px var(--ac-one);
		}
	}
